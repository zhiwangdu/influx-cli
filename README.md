...
TSSP record execution diagnostics distinguish total local record candidate rows, output rows, range-rejected rows, filter-rejected rows, and sampled execution rows; samples include the query range context, decoded value-column count, and local output ordinal for each materialized local record row.

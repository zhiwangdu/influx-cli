...
influx-cli --format json storage analyze /path/to/_series --series-id 42
...
influx-cli --format json storage analyze /path/to/opengemini-meta.pb --storage-format opengemini-meta
...
`tsi-log` JSON output includes local live/tombstone series-id counts and min/max ranges after replaying local log series and tombstone entries.
...
Filter execution decision samples report scalar, string, and regex predicate values after local quoted-literal unescaping, while set and range predicates keep the original local list or bound text.

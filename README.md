...
Default table `details` cells also include sorted local `block_types` counts when an analyzer records per-block categories; `<file-set>` aggregate rows include the cross-file block-type rollup. Default `decode_path` cells include local query range/seek context, target/match/missing counts, block filter skip counts, sorted local location/decode block-type rollups, decode byte/value/output count comparisons, cursor/window/execution sample counts, TSSP data probe checked/valid/failure and failure-breakdown counts, failure reason rollups, row-count/output materialization counts, probed block-type rollups, and unavailable-value reason rollups when those local planning/probe stages run.

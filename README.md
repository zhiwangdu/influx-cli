...
Default table `details` cells also include sorted local `block_types` counts when an analyzer records per-block categories; `<file-set>` aggregate rows include the cross-file block-type rollup.

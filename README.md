...
Default table `details` cells also include sorted local `block_types` counts when an analyzer records per-block categories; `<file-set>` aggregate rows include the cross-file block-type rollup. Default `decode_path` cells include sorted local location/decode block-type rollups when TSM/TSSP/mergeset planning records them, plus TSSP data probe checked/valid/failure and failure-breakdown counts, failure reason rollups, row-count/output materialization counts, probed block-type rollups, and unavailable-value reason rollups when data block probes run.

...
For mergeset JSON output, item payload extras include plain/zstd `items.bin` plus `lens.bin` read bytes and successful uncompressed payload bytes; failed decodes keep read-byte and failure counts without estimating partial uncompressed bytes.

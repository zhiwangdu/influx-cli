...
TSSP record execution diagnostics distinguish total local record candidate rows, output rows, range-rejected rows, filter-rejected rows, and sampled execution rows; execution samples include the query range context and decoded value-column count, while both record output and execution samples include the local output ordinal for each materialized local record row.

...
Use `--storage-format tsm|tssp|tsi` to override auto-detection when needed. Repeat `--key` to scope TSM decode-path planning to specific TSM index keys. Use `--cursor-order asc|desc` to model ascending or descending TSM/openGemini TSSP cursor planning. Repeat `--series-id` to scope openGemini TSSP decode-path planning to specific series IDs. Repeat `--measurement` and `--tag key=value` to inspect TSI measurement/tag predicates.

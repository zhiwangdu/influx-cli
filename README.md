...
For mergeset JSON output, item payload extras include plain/zstd `items.bin` plus `lens.bin` read bytes and successful uncompressed payload bytes; failed decodes keep read-byte and failure counts without estimating partial uncompressed bytes. Mergeset cursor output and heap execution samples add `key_hex`, `optimized_value_hex`, or `candidate_value_hex` when a sampled item contains non-printable binary bytes.

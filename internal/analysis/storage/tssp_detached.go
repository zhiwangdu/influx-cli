...
func analyzeTSSPDetachedMetaIndex(path string, info os.FileInfo, options Options) (FileReport, error) {
	...
	if expanded {
		...
		if err != nil {
			...
			report.Extra["data_block_probe_record_rows"] = fmt.Sprint(dataProbe.RecordRows)
			...
			report.Extra["data_block_probe_record_range_rejects"] = fmt.Sprint(dataProbe.RecordRangeRejects)
			report.Extra["data_block_probe_record_filter_rejects"] = fmt.Sprint(dataProbe.RecordFilterRejects)
			...
			report.Extra["data_block_probe_filter_short_circuit_skips"] = fmt.Sprint(dataProbe.FilterSkippedEvals)
			report.Extra["data_block_probe_required_filter_evaluation_matches"] = fmt.Sprint(dataProbe.FilterRequiredHits)
			report.Extra["data_block_probe_required_filter_evaluation_misses"] = fmt.Sprint(dataProbe.FilterRequiredMiss)
			report.Extra["data_block_probe_required_filter_short_circuit_skips"] = fmt.Sprint(dataProbe.FilterRequiredSkips)
			report.Extra["data_block_probe_any_filter_evaluation_matches"] = fmt.Sprint(dataProbe.FilterAnyHits)
			report.Extra["data_block_probe_any_filter_evaluation_misses"] = fmt.Sprint(dataProbe.FilterAnyMiss)
			report.Extra["data_block_probe_any_filter_short_circuit_skips"] = fmt.Sprint(dataProbe.FilterAnySkips)
			report.Extra["data_block_probe_none_filter_evaluation_matches"] = fmt.Sprint(dataProbe.FilterNoneHits)
			report.Extra["data_block_probe_none_filter_evaluation_misses"] = fmt.Sprint(dataProbe.FilterNoneMiss)
			report.Extra["data_block_probe_none_filter_short_circuit_skips"] = fmt.Sprint(dataProbe.FilterNoneSkips)
...
func appendTSSPDetachedDataProbeValueSamples(probe *tsspDetachedDataProbe, chunk tsspChunkMeta, timeRange tsspTimeRange, blocks map[string]tsspDetachedDataBlockInfo, matchingRows []bool, queryRange TimeRange, sampleLimit int) {
	...
	probe.valueSamples, recordExecutionSamples, recordStats = appendTSSPDataProbeRecordSamples(probe.valueSamples, "meta-index-id", chunk.SID, timeRange, blocks, matchingRows, queryRange, sampleLimit, remainingTSSPExecutionSampleLimit(probe.recordExecutionSamples, sampleLimit))
		...
		if sampled {
			...
			if recordStepLimit > 0 && len(recordSteps) < recordStepLimit {
				recordSteps = append(recordSteps, DecodePathCursorStep{
					...
					CandidateValue:    tsspRecordExecutionCandidateValue(row, stats.Outputs, timestamp, len(columnNames), values, queryRange),
...
func tsspDetachedChunkDecodeRecommendations(summary *DecodePathSummary) []string {
	...
	if len(summary.MissingNoneFields) > 0 {
		recommendations = append(recommendations, fmt.Sprintf(
			"%d query NOT field filter(s) were not found in analyzed detached TSSP chunk metadata",
	...
	if len(summary.QueryNoneFields) > 0 {
		recommendations = append(recommendations, fmt.Sprintf(
			"applied %d detached TSSP NOT field filter(s) to decoded record rows",
	...
	if summary.DataBlockProbeRangeRows > 0 {
		recommendations = append(recommendations, fmt.Sprintf(
			"detached TSSP query range matched %d of %d decoded row timestamp(s)",
	...
	if summary.DataBlockProbeFilterEvals > 0 {
		recommendations = append(recommendations, fmt.Sprintf(
			"executed %d detached TSSP decoded-row field predicate evaluation(s): matches=%d misses=%d required=%d required_matches=%d required_misses=%d any=%d any_matches=%d any_misses=%d none=%d none_matches=%d none_misses=%d",
	...
	if summary.DataBlockProbeRecordOutputs > 0 {
		recommendations = append(recommendations, fmt.Sprintf(
			"materialized %d detached TSSP record output row(s) from decoded column blocks with %d sampled",
	...
	if summary.DataBlockProbeRecordRangeRejects > 0 || summary.DataBlockProbeRecordFilterRejects > 0 {
		recommendations = append(recommendations, fmt.Sprintf(
			"rejected %d detached TSSP record row(s) during local materialization: range=%d filters=%d",

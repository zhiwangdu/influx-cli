...
func TestTSSPDetachedFileSetDecodePathSummarizesTotalExecutionActions(t *testing.T) {
	...
	if got, want := summary.FilterClauseTotalActions["filter_required_miss"], 1; got != want {
	...
	if got, want := summary.FilterClauseTotalActions["filter_any_skip"], 2; got != want {
...
func TestAnalyzeTSSPDetachedSamplesFloatFullUnsupportedCodecReason(t *testing.T) {
	dir := t.TempDir()
	timestamps := []int64{100, 110}
	valueSize := testTSSPDetachedUnsupportedFloatFullBlockSize(len(timestamps), 7)
	chunks := []testTSSPChunkSpec{{
		sid:      43,
		minTime:  timestamps[0],
		maxTime:  timestamps[len(timestamps)-1],
	...
	if err := writeTestTSSPDetachedUnsupportedFloatFullData(filepath.Join(dir, tsspDetachedDataFileName), 1600, chunks[0], len(timestamps), 7, timestamps); err != nil {
		t.Fatal(err)
	}
	queryRange, err := NewTimeRange(timestamps[0], timestamps[len(timestamps)-1])
	if err != nil {
	...
	if !containsString(decode.Recommendations, "detached TSSP data block probe found 1 block(s) with unavailable value samples: float-full-codec-7:1") {
...
func TestAnalyzeTSSPDetachedSamplesLZ4StringFullBlocks(t *testing.T) {
	dir := t.TempDir()
	values := []string{
		strings.Repeat("red-", 32),
		strings.Repeat("blue-", 24),
	}
	timestamps := []int64{700, 730}
	valueSize, err := testTSSPDetachedStringFullBlockSizeWithCodec(values, 3)
	if err != nil {
		t.Fatal(err)
	}
	chunks := []testTSSPChunkSpec{{
	...
	if err := writeTestTSSPDetachedStringFullDataWithCodec(filepath.Join(dir, tsspDetachedDataFileName), 1800, chunks[0], values, 3, timestamps); err != nil {
		t.Fatal(err)
	}
	queryRange, err := NewTimeRange(timestamps[0], timestamps[len(timestamps)-1])
	if err != nil {
...
func TestAnalyzeTSSPDetachedSamplesBooleanFullBitpackBlocks(t *testing.T) {
	dir := t.TempDir()
	values := []bool{true, false, true}
	timestamps := []int64{800, 830, 860}
	valueSize := testTSSPDetachedBooleanFullBlockSize(values)
	chunks := []testTSSPChunkSpec{{
		sid:      46,
		minTime:  timestamps[0],
		maxTime:  timestamps[len(timestamps)-1],
	...
	if err := writeTestTSSPDetachedBooleanFullData(filepath.Join(dir, tsspDetachedDataFileName), 1800, chunks[0], values, timestamps); err != nil {
		t.Fatal(err)
	}
	queryRange, err := NewTimeRange(timestamps[0], timestamps[len(timestamps)-1])
	if err != nil {
...
func TestAnalyzeTSSPDetachedAnyFieldFilterMatchesEitherPredicate(t *testing.T) {
	...
	if got, want := len(decode.FilterClauseTotalActions), 3; got != want {
		t.Fatalf("filter clause total action count entries = %d, want %d: %+v", got, want, decode.FilterClauseTotalActions)
	}
	if got, want := decode.FilterClauseTotalActions["filter_any_match"], 1; got != want {
		t.Fatalf("total filter_any_match action count = %d, want %d", got, want)
	}
	...
	wantFilterStep := DecodePathCursorStep{
		...
		Key:               "meta-index-id:42/row:0",
		CandidateValue:    "row=0 time=333 required=0/0 any=1/2 none=0/0 skips=0/1/0 values=value=99 decisions=any:value:=:0:miss;any:value:=:99:match result=match",
		CursorIndexBefore: 0,
		CursorIndexAfter:  1,
		CursorAdvanced:    true,
	}
...
func TestAnalyzeTSSPDetachedNoneFieldFilterRejectsMatchingRows(t *testing.T) {
	...
	wantFilterStep := DecodePathCursorStep{
		...
		Key:               "meta-index-id:42/row:0",
		CandidateValue:    "row=0 time=333 required=0/0 any=0/0 none=1/1 skips=0/0/0 values=value=99 decisions=none:value:=:99:match result=reject_none",
		CursorIndexBefore: 0,
		CursorIndexAfter:  1,
		CursorAdvanced:    true,
	}
...
func TestAnalyzeTSSPDetachedAnyAndNoneFieldFiltersMatchDecodedTime(t *testing.T) {
	dir := t.TempDir()
	values := []string{"red", "blue"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
	...
	report, err := Analyze(context.Background(), []string{filepath.Join(dir, tsspDetachedMetaIndexFileName)}, Options{
		...
		QueryRange:       queryRange,
		QueryColumns:     []string{"value"},
		QueryAnyFields:   []FieldFilter{{Key: "time", Value: fmt.Sprint(timestamps[0])}, {Key: "value", Value: "blue"}},
		QueryNoneFields:  []FieldFilter{{Key: "time", Value: fmt.Sprint(timestamps[1])}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	wantAny := []FieldFilter{{Key: "time", Value: fmt.Sprint(timestamps[0])}, {Key: "value", Value: "blue"}}
	if got := decode.QueryAnyFields; !equalFieldFilters(got, wantAny) {
		t.Fatalf("query any fields = %v, want %v", got, wantAny)
	}
	if got := decode.MatchedAnyFields; !equalFieldFilters(got, wantAny) {
	...
	wantNone := []FieldFilter{{Key: "time", Value: fmt.Sprint(timestamps[1])}}
	if got := decode.QueryNoneFields; !equalFieldFilters(got, wantNone) {
		t.Fatalf("query none fields = %v, want %v", got, wantNone)
	}
	if got := decode.MatchedNoneFields; !equalFieldFilters(got, wantNone) {
	...
	if got, want := decode.DataBlockProbeAnyEvals, 3; got != want {
		t.Fatalf("decode any filter evaluations = %d, want %d", got, want)
	}
	...
	for i, want := range []DecodePathCursorStep{
		{
			...
			Action:            "filter_row_match",
			Key:               "meta-index-id:42/row:0",
			CandidateValue:    "row=0 time=333 required=0/0 any=1/1 none=0/1 skips=0/1/0 values=time=333,value=red decisions=any:time:=:333:match;none:time:=:444:miss result=match",
			CursorIndexBefore: 0,
			CursorIndexAfter:  1,
			CursorAdvanced:    true,
		},
		{
			...
			Action:            "filter_row_reject_none",
			Key:               "meta-index-id:42/row:1",
			CandidateValue:    "row=1 time=444 required=0/0 any=1/2 none=1/1 skips=0/0/0 values=time=444,value=blue decisions=any:time:=:333:miss;any:value:=:blue:match;none:time:=:444:match result=reject_none",
			CursorIndexBefore: 1,
			CursorIndexAfter:  2,
			CursorAdvanced:    true,
		},
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesQuotedStringInSet(t *testing.T) {
	dir := t.TempDir()
	values := []string{"red,primary", "blue)"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
	if err != nil {
		t.Fatal(err)
	...
	if got, want := file.Extra["data_block_probe_filter_operator_evaluations"], "in:2"; got != want {
		t.Fatalf("data block probe filter operator evaluations = %q, want %q", got, want)
	}
	decode := file.DecodePath
	if decode == nil {
	...
	if got, want := decode.QueryFields, []FieldFilter{{Key: "value", Op: "in", Value: `("red,primary","blue)")`}}; !equalFieldFilters(got, want) {
		t.Fatalf("query fields = %v, want %v", got, want)
	}
	if got, want := decode.OptimizedValueOutputPoints, 2; got != want {
		t.Fatalf("optimized value output points = %d, want %d", got, want)
	...
	for i, want := range []DecodePathCursorOutput{
		{Key: "meta-index-id:42/value", Time: 333, Type: "string-full", OptimizedValue: "red,primary", Matches: true},
		{Key: "meta-index-id:42/value", Time: 444, Type: "string-full", OptimizedValue: "blue)", Matches: true},
	} {
		got := decode.CursorOutputSamples[i]
		if got != want {
			t.Fatalf("cursor output sample %d = %+v, want %+v", i, got, want)
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesQuoteWrappedStringInSet(t *testing.T) {
	dir := t.TempDir()
	values := []string{`"red"`, "plain"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
	if err != nil {
		t.Fatal(err)
	...
	if got, want := decode.QueryFields, []FieldFilter{{Key: "value", Op: "in", Value: `("\"red\"")`}}; !equalFieldFilters(got, want) {
		t.Fatalf("query fields = %v, want %v", got, want)
	}
	if got, want := decode.OptimizedValueOutputPoints, 1; got != want {
		t.Fatalf("optimized value output points = %d, want %d", got, want)
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesQuotedStringEquality(t *testing.T) {
	dir := t.TempDir()
	values := []string{"red,primary", "blue)"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
	...
	if got, want := decode.QueryFields, []FieldFilter{{Key: "value", Value: `"red,primary"`}}; !equalFieldFilters(got, want) {
		t.Fatalf("query fields = %v, want %v", got, want)
	}
	if got, want := decode.OptimizedValueOutputPoints, 1; got != want {
		t.Fatalf("optimized value output points = %d, want %d", got, want)
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesInequalityWordAlias(t *testing.T) {
	dir := t.TempDir()
	...
	report, err := Analyze(context.Background(), []string{filepath.Join(dir, tsspDetachedMetaIndexFileName)}, Options{
		...
		BlockSampleLimit: 4,
		QueryRange:       queryRange,
		QueryFields:      []FieldFilter{{Key: "value", Op: "not equals", Value: "fail"}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if got, want := decode.QueryFields, []FieldFilter{{Key: "value", Op: "!=", Value: "fail"}}; !equalFieldFilters(got, want) {
		t.Fatalf("query fields = %v, want %v", got, want)
	}
	if got, want := decode.OptimizedValueOutputPoints, 1; got != want {
		t.Fatalf("optimized value output points = %d, want %d", got, want)
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesBooleanNotInSet(t *testing.T) {
	dir := t.TempDir()
	times, err := writeTestTSSPDetachedMultiColumnRecordData(dir)
	if err != nil {
		t.Fatal(err)
	...
	if got, want := file.Extra["data_block_probe_filter_operator_evaluations"], "not-in:2"; got != want {
		t.Fatalf("data block probe filter operator evaluations = %q, want %q", got, want)
	}
	decode := file.DecodePath
	if decode == nil {
	...
	if got, want := decode.QueryColumns, []string{"status"}; !equalStrings(got, want) {
		t.Fatalf("query columns = %v, want %v", got, want)
	}
...
func TestAnalyzeTSSPDetachedFieldFilterPreservesSetRangeValueSymbols(t *testing.T) {
	dir := t.TempDir()
	...
	if err := writeTestTSSPDetachedStringFullData(filepath.Join(dir, tsspDetachedDataFileName), 1500, chunks[0], values, timestamps); err != nil {
		t.Fatal(err)
	}
	...
	report, err := Analyze(context.Background(), []string{filepath.Join(dir, tsspDetachedMetaIndexFileName)}, Options{
		...
		QueryFields: []FieldFilter{
			{Key: "value", Op: "in", Value: "(a=b,c<>d)"},
			{Key: "value", Op: "between", Value: "a=0,c=z"},
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	if got, want := file.Extra["data_block_probe_filter_operator_evaluations"], "between:3,in:2"; got != want {
		t.Fatalf("data block probe filter operator evaluations = %q, want %q", got, want)
	}
	decode := file.DecodePath
	if decode == nil {
	...
	for op, want := range map[string]int{"in": 2, "between": 3} {
	...
	if got, want := len(decode.FilterExecutionSamples), 3; got != want {
		t.Fatalf("filter execution samples = %d, want %d", got, want)
	}
	for i, want := range []DecodePathCursorStep{
		{
			...
			Action:            "filter_row_match",
			Key:               "meta-index-id:42/row:0",
			CandidateValue:    "row=0 time=100 required=2/2 any=0/0 none=0/0 skips=0/0/0 values=value=a\\=b decisions=required:value:between:a=0,c=z:match;required:value:in:(a=b,c<>d):match result=match",
			CursorIndexBefore: 0,
			CursorIndexAfter:  1,
			CursorAdvanced:    true,
		},
		{
			...
			Action:            "filter_row_match",
			Key:               "meta-index-id:42/row:1",
			CandidateValue:    "row=1 time=200 required=2/2 any=0/0 none=0/0 skips=0/0/0 values=value=c<>d decisions=required:value:between:a=0,c=z:match;required:value:in:(a=b,c<>d):match result=match",
			CursorIndexBefore: 1,
			CursorIndexAfter:  2,
			CursorAdvanced:    true,
		},
		{
			...
			Action:            "filter_row_reject_required",
			Key:               "meta-index-id:42/row:2",
			CandidateValue:    "row=2 time=300 required=0/1 any=0/0 none=0/0 skips=1/0/0 values=value=z!x decisions=required:value:between:a=0,c=z:miss result=reject_required",
			CursorIndexBefore: 2,
			CursorIndexAfter:  3,
			CursorAdvanced:    true,
		},
	...
	for i, want := range []DecodePathCursorOutput{
		{Key: "meta-index-id:42/value", Time: 100, Type: "string-full", OptimizedValue: "a=b", Matches: true},
		{Key: "meta-index-id:42/value", Time: 200, Type: "string-full", OptimizedValue: "c<>d", Matches: true},
	} {
		if got := decode.CursorOutputSamples[i]; got != want {
			t.Fatalf("cursor output sample[%d] = %+v, want %+v", i, got, want)
		}
...
func TestAnalyzeTSSPDetachedFilterExecutionReportsUnquotedScalarDecisionValues(t *testing.T) {
	dir := t.TempDir()
	...
	report, err := Analyze(context.Background(), []string{filepath.Join(dir, tsspDetachedMetaIndexFileName)}, Options{
		...
		BlockSampleLimit: 8,
		QueryRange:       queryRange,
		QueryFields:      []FieldFilter{{Key: "value", Op: "equals", Value: `"ok value"`}},
		QueryAnyFields:   []FieldFilter{{Key: "value", Op: "=~", Value: `"ok.*"`}},
		QueryNoneFields:  []FieldFilter{{Key: "value", Op: "contains", Value: `"debug:trace;1"`}},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if got, want := file.Extra["data_block_probe_filter_operator_evaluations"], "=:2,=~:1,contains:1"; got != want {
		t.Fatalf("data block probe filter operator evaluations = %q, want %q", got, want)
	}
	decode := file.DecodePath
	if decode == nil {
	...
	if got, want := decode.QueryFields, []FieldFilter{{Key: "value", Value: `"ok value"`}}; !equalFieldFilters(got, want) {
		t.Fatalf("query fields = %v, want %v", got, want)
	}
	if got, want := decode.QueryAnyFields, []FieldFilter{{Key: "value", Op: "=~", Value: `"ok.*"`}}; !equalFieldFilters(got, want) {
		t.Fatalf("query any fields = %v, want %v", got, want)
	}
	if got, want := decode.QueryNoneFields, []FieldFilter{{Key: "value", Op: "contains", Value: `"debug:trace;1"`}}; !equalFieldFilters(got, want) {
		t.Fatalf("query none fields = %v, want %v", got, want)
	}
	...
	wantFilterSteps := []DecodePathCursorStep{
		{
			Step:              1,
			Type:              "tssp-filter-row-step",
			Action:            "filter_row_match",
			Key:               "meta-index-id:42/row:0",
			CandidateValue:    "row=0 time=333 required=1/1 any=1/1 none=0/1 skips=0/0/0 values=value=ok\\svalue decisions=required:value:=:ok\\svalue:match;any:value:=~:ok.*:match;none:value:contains:debug\\:trace\\;1:miss result=match",
			CursorIndexBefore: 0,
			CursorIndexAfter:  1,
			CursorAdvanced:    true,
		},
		{
			...
			Action:            "filter_row_reject_required",
			Key:               "meta-index-id:42/row:1",
			CandidateValue:    "row=1 time=444 required=0/1 any=0/0 none=0/0 skips=0/1/1 values=value=debug:trace;1 decisions=required:value:=:ok\\svalue:miss result=reject_required",
			CursorIndexBefore: 1,
			CursorIndexAfter:  2,
			CursorAdvanced:    true,
		},
	}
	if got, want := len(decode.FilterExecutionSamples), len(wantFilterSteps); got != want {
		t.Fatalf("filter execution samples = %d, want %d", got, want)
	}
	for i, want := range wantFilterSteps {
		if got := decode.FilterExecutionSamples[i]; got != want {
			t.Fatalf("filter execution sample[%d] = %+v, want %+v", i, got, want)
		}
	}
	if got, want := decode.FilterExecutionActions["filter_row_match"], 1; got != want {
		t.Fatalf("filter row match actions = %d, want %d", got, want)
	}
	if got, want := decode.FilterExecutionActions["filter_row_reject_required"], 1; got != want {
		t.Fatalf("filter row reject required actions = %d, want %d", got, want)
	}
	if !containsString(decode.Recommendations, "detached TSSP filter execution samples show local decoded-row predicate decisions") {
		t.Fatalf("recommendations = %v, want detached predicate execution sample recommendation", decode.Recommendations)
	}
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesStringLike(t *testing.T) {
	dir := t.TempDir()
	values := []string{"red", "blue"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesStringPrefixSuffix(t *testing.T) {
	dir := t.TempDir()
	values := []string{"red", "blue"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
...
func TestAnalyzeTSSPDetachedFieldFilterMatchesStringCaseInsensitiveOperators(t *testing.T) {
	dir := t.TempDir()
	values := []string{"red", "blue"}
	timestamps := []int64{333, 444}
	valueSize, err := testTSSPDetachedStringFullBlockSize(values)
	...
	report, err := Analyze(context.Background(), []string{filepath.Join(dir, tsspDetachedMetaIndexFileName)}, Options{
		...
		QueryRange:       queryRange,
		QueryFields: []FieldFilter{
			{Key: "value", Op: "icontains", Value: "L"},
			{Key: "value", Op: "not icontains", Value: "R"},
			{Key: "value", Op: "ilike", Value: "B%"},
			{Key: "value", Op: "not ilike", Value: "R_D"},
			{Key: "value", Op: "istarts_with", Value: "B"},
			{Key: "value", Op: "not_istarts_with", Value: "R"},
			{Key: "value", Op: "iends-with", Value: "E"},
			{Key: "value", Op: "not_iends_with", Value: "D"},
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	wantFields := []FieldFilter{
		...
		{Key: "value", Op: "iends-with", Value: "E"},
		{Key: "value", Op: "ilike", Value: "B%"},
		{Key: "value", Op: "istarts-with", Value: "B"},
		{Key: "value", Op: "not-icontains", Value: "R"},
		{Key: "value", Op: "not-iends-with", Value: "D"},
		{Key: "value", Op: "not-ilike", Value: "R_D"},
		{Key: "value", Op: "not-istarts-with", Value: "R"},
	}
	if got := decode.QueryFields; !equalFieldFilters(got, wantFields) {
		t.Fatalf("query fields = %v, want %v", got, wantFields)
	}
...
func writeTestTSSPDetachedStringFullData(path string, size int, chunk testTSSPChunkSpec, values []string, timestamps []int64) error {
	return writeTestTSSPDetachedStringFullDataWithCodec(path, size, chunk, values, 0, timestamps)
}

func writeTestTSSPDetachedStringFullDataWithCodec(path string, size int, chunk testTSSPChunkSpec, values []string, codec byte, timestamps []int64) error {
	valuePayload, err := testTSSPStringFullPayload(values, codec)
	if err != nil {
		return err
	}
...
func writeTestTSSPDetachedUnsupportedFloatFullData(path string, size int, chunk testTSSPChunkSpec, rows int, codec byte, timestamps []int64) error {
	var valuePayload bytes.Buffer
	writeTestTSSPAttachedUnsupportedFloatFullBlock(&valuePayload, rows, codec)
	timePayload := testTSSPIntegerFullPayload(timestamps)
	var buf bytes.Buffer
	buf.WriteString(tsspMagic)
	writeUint64(&buf, 2)
...
func writeTestTSSPDetachedBooleanFullData(path string, size int, chunk testTSSPChunkSpec, values []bool, timestamps []int64) error {
	valuePayload := testTSSPBooleanFullPayload(values)
	timePayload := testTSSPIntegerFullPayload(timestamps)
	var buf bytes.Buffer
	buf.WriteString(tsspMagic)
	writeUint64(&buf, 2)
...
func testTSSPDetachedStringFullBlockSize(values []string) (uint32, error) {
	return testTSSPDetachedStringFullBlockSizeWithCodec(values, 0)
}

func testTSSPDetachedStringFullBlockSizeWithCodec(values []string, codec byte) (uint32, error) {
	payload, err := testTSSPStringFullPayload(values, codec)
	if err != nil {
		return 0, err
	}
...
func testTSSPDetachedUnsupportedFloatFullBlockSize(rows int, codec byte) uint32 {
	var payload bytes.Buffer
	writeTestTSSPAttachedUnsupportedFloatFullBlock(&payload, rows, codec)
	return uint32(crc32.Size + payload.Len())
}
...
func testTSSPDetachedBooleanFullBlockSize(values []bool) uint32 {
	return uint32(crc32.Size + len(testTSSPBooleanFullPayload(values)))
}
...
func testTSSPBooleanFullPayload(values []bool) []byte {
	var payload bytes.Buffer
	writeTestTSSPAttachedBooleanFullBlock(&payload, values)
	return payload.Bytes()
}

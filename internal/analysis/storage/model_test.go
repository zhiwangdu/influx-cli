...
func TestReportResultIncludesTSSPDecodePathSummary(t *testing.T) {
	report := Report{
		Files: []FileReport{{
			...
			DecodePath: &DecodePathSummary{
				...
				RangeExecutionOmittedActions: map[string]int{
					"range_row_reject": 1,
				},
				FilterExecutionTotalActions: map[string]int{
					"filter_row_match":  3,
					"filter_row_reject": 2,
				},
				FilterExecutionOmittedActions: map[string]int{
					"filter_row_reject": 1,
				},
	...
	for _, want := range []string{
		...
		"execution windows cursor=2 sampled=1 merge=1 merge_blocks=2 merge_keys=1 samples range_total_actions range_row_match:5 range_row_reject:2 range_omitted_actions range_row_reject:1 record_total_actions record_row_filter_reject:2 record_row_output:2 record_row_range_reject:1 filter_total_actions filter_row_match:3 filter_row_reject:2 filter_omitted_actions filter_row_reject:1 filter_clause_total_actions filter_any_skip:4 decisions=1 cursor_steps=1 cursor_actions push:1 filter_steps=1 filter_actions match:1 amplification=2.50x",
...
func TestDecodePathTextIncludesExecutionDiagnosticsCountsOnly(t *testing.T) {
	...
	want := "execution windows cursor=3 sampled=2 merge=2 merge_blocks=5 merge_keys=4 samples decisions=1 cursor_steps=2 cursor_actions advance:1 push:1 record_steps=2 record_actions record_row_filter_reject:1 record_row_output:1 filter_steps=1 filter_actions match:1 amplification=1.75x"
	if !strings.Contains(text, want) {
		t.Fatalf("decode path text = %q, want %q", text, want)
	}
...
func TestDecodePathTextUsesRecordExecutionActionCounts(t *testing.T) {
	summary := DecodePathSummary{
		CursorExecutionActions: map[string]int{
			"read_segments": 2,
		},
		...
		RecordExecutionOmittedActions: map[string]int{
			"record_row_output":       4,
			"record_row_range_reject": 2,
		},
		FilterExecutionTotalActions: map[string]int{
	...
	want := "execution samples range_total_actions range_row_match:5 range_row_reject:2 range_omitted_actions range_row_match:4 range_row_reject:2 record_total_actions record_row_filter_reject:1 record_row_output:6 record_row_range_reject:2 record_omitted_actions record_row_output:4 record_row_range_reject:2 filter_total_actions filter_row_match:3 filter_row_reject:2 filter_omitted_actions filter_row_match:2 filter_row_reject:2 filter_clause_total_actions filter_any_skip:4 filter_required_match:1 filter_required_miss:2 cursor_actions read_segments:2 range_actions range_row_match:1 record_actions record_row_filter_reject:1 record_row_output:2 filter_actions filter_row_match:1"
	if !strings.Contains(text, want) {
		t.Fatalf("decode path text = %q, want %q", text, want)
	}
	...
	var decoded struct {
		...
		RangeExecutionOmittedActions  map[string]int `json:"range_execution_omitted_action_counts"`
		...
		RecordExecutionOmittedActions map[string]int `json:"record_execution_omitted_action_counts"`
		...
		FilterExecutionOmittedActions map[string]int `json:"filter_execution_omitted_action_counts"`
		FilterClauseTotalActions      map[string]int `json:"filter_clause_total_action_counts"`
	}
	if err := json.Unmarshal(data, &decoded); err != nil {
		t.Fatal(err)
	}
	if got, want := decoded.CursorExecutionActions["read_segments"], 2; got != want {
		t.Fatalf("read_segments count = %d, want %d", got, want)
	}
	...
	if got, want := decoded.RangeExecutionOmittedActions["range_row_match"], 4; got != want {
		t.Fatalf("omitted range_row_match count = %d, want %d", got, want)
	}
	...
	if got, want := decoded.RecordExecutionOmittedActions["record_row_output"], 4; got != want {
		t.Fatalf("omitted record_row_output count = %d, want %d", got, want)
	}
	...
	if got, want := decoded.FilterExecutionOmittedActions["filter_row_reject"], 2; got != want {
		t.Fatalf("omitted filter_row_reject count = %d, want %d", got, want)
	}

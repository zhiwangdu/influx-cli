...
func TestReportResultIncludesTSSPDecodePathSummary(t *testing.T) {
	...
	for _, want := range []string{
		...
		"data_probe blocks=4 bytes=256 failures=1 crc_mismatches=1 row_blocks=3 row_unknowns=1 row_mismatches=1 output_points=2 value_blocks=2 value_unknowns=1 nulls=3 record_samples=1",
...
func TestReportResultIncludesReportLevelDecodePathSummary(t *testing.T) {
	...
	for _, want := range []string{
		...
		"data_probe blocks=6 bytes=0 failures=0 crc_mismatches=0 row_blocks=4 row_unknowns=1 row_mismatches=1 output_points=5 value_blocks=0 value_unknowns=0 nulls=0 record_samples=0",

...
func TestAnalyzeTSMFileStoreFinalOutputSamplesFallBackToOlderFileOnTombstonedPoints(t *testing.T) {
	dir := t.TempDir()
	path1 := filepath.Join(dir, "000000001-000000001.tsm")
	if err := writeTestTSMWithBlocks(path1, []testTSMBlockSpec{
		{key: "cpu,host=a value", typ: tsmBlockInteger, minTime: 10, maxTime: 30, timestamps: []int64{10, 10, 10}, values: []int64{1, 2, 3}},
	...
	if err := writeTestTSMTombstoneV3(tsmTombstonePath(path2),
		tsmTombstoneEntry{Key: "cpu,host=a value", Min: 20, Max: 30},
	); err != nil {
		t.Fatal(err)
	}
	...
	if got, want := decode.FullyTombstonedBlocks, 0; got != want {
		t.Fatalf("fully tombstoned blocks = %d, want %d", got, want)
	}
	if got, want := decode.BaselineValueOutputPoints, 2; got != want {
	...
	for i, want := range []DecodePathCursorOutput{
		{Key: "cpu,host=a value", Time: 20, Type: "integer", File: path1, OptimizedValue: "2", Matches: true},
		{Key: "cpu,host=a value", Time: 30, Type: "integer", File: path1, OptimizedValue: "3", Matches: true},
	} {
		got := decode.CursorFinalOutputSamples[i]
		if got != want {
			t.Fatalf("cursor final output sample[%d] = %+v, want %+v", i, got, want)
...
func TestAnalyzeTSMFileStoreFinalOutputSamplesFallBackToOlderFileOnDescendingTombstonedPoints(t *testing.T) {
	dir := t.TempDir()
	path1 := filepath.Join(dir, "000000001-000000001.tsm")
	if err := writeTestTSMWithBlocks(path1, []testTSMBlockSpec{
		{key: "cpu,host=a value", typ: tsmBlockInteger, minTime: 10, maxTime: 30, timestamps: []int64{10, 10, 10}, values: []int64{1, 2, 3}},
...
func TestAnalyzeTSMDecodePathComparesUnsignedValueOutput(t *testing.T) {
	path := filepath.Join(t.TempDir(), "000000001-000000001.tsm")
	unsignedHigh := uint64(1) << 63
	if err := writeTestTSMWithBlocks(path, []testTSMBlockSpec{
		{key: "bytes,host=a value", typ: tsmBlockUnsigned, minTime: 10, maxTime: 30, timestamps: []int64{10, 10, 10}, unsignedValues: []uint64{unsignedHigh + 1, unsignedHigh + 2, unsignedHigh + 3}},
		{key: "bytes,host=a value", typ: tsmBlockUnsigned, minTime: 20, maxTime: 40, timestamps: []int64{20, 10, 10}, unsignedValues: []uint64{unsignedHigh + 20, unsignedHigh + 30, unsignedHigh + 40}},
	}); err != nil {
		t.Fatal(err)
	}
	...
	for i, want := range []DecodePathCursorOutput{
		{Key: "bytes,host=a value", Time: 20, Type: "unsigned", OptimizedValue: "9223372036854775828", Matches: true},
		{Key: "bytes,host=a value", Time: 30, Type: "unsigned", OptimizedValue: "9223372036854775838", Matches: true},
	} {
		got := decode.CursorFinalOutputSamples[i]
		if got != want {
			t.Fatalf("cursor final output sample[%d] = %+v, want %+v", i, got, want)
	...
	for timestamp, want := range map[int64]string{20: "9223372036854775828", 30: "9223372036854775838"} {
		point, ok := baseline[tsmOutputPointKey{key: "bytes,host=a value", timestamp: timestamp, typ: tsmBlockUnsigned}]
		if !ok {
			t.Fatalf("missing merged point at %d", timestamp)
		}
		if point.Value != want {
...
func TestAnalyzeAutoDetectsStorageFormats(t *testing.T) {
	...
	tsiLogPath := filepath.Join(dir, "L0-00000001.tsl")
	var tsiLog bytes.Buffer
	appendTestTSILogEntry(&tsiLog, tsiLogEntry{SeriesID: 1})
	if err := os.WriteFile(tsiLogPath, tsiLog.Bytes(), 0o600); err != nil {
		t.Fatal(err)
	}

	report, err := Analyze(context.Background(), []string{tsmPath, tsspPath, tsiPath, tsiLogPath}, Options{
		Format:           FormatAuto,
		KeySampleLimit:   1,
		BlockSampleLimit: 1,
	})
	...
	if got, want := len(report.Files), 4; got != want {
		t.Fatalf("file count = %d, want %d", got, want)
	}
	...
	if !formats[FormatTSM] || !formats[FormatTSSP] || !formats[FormatTSI] || !formats[FormatTSILog] {
		t.Fatalf("formats = %v, want %s, %s, %s, and %s", formats, FormatTSM, FormatTSSP, FormatTSI, FormatTSILog)
	}
}
...
func testTSMBlock(spec testTSMBlockSpec) []byte {
	...
	case spec.typ == tsmBlockUnsigned && len(spec.unsignedValues) > 0:
		block.Write(testTSMUnsignedValueBlock(spec.unsignedValues))

...
当前覆盖 InfluxDB TSM attached file metadata、tombstone range/impact summary、基于 query range/key 的 TSM decode path estimate、per-file 和 FileStore-level cursor window/merge window simulation、优化前后 block/byte/value decode path estimate、decoded timestamp output/dedup estimate、TSM value output comparison、TSI index file measurement/tag summary 和 measurement/tag predicate inspection，以及 openGemini attached TSSP trailer/meta-index metadata 和 none/snappy/LZ4/self-compressed chunk metadata。完整 storage-engine cursor execution 仍属于后续 Phase 5 工作。

...
TSSP multi-word field predicate operator alias 会接受 hyphen、space 和 underscore 分隔符，bang-negated prefix/suffix alias 也接受 underscore 形式；record execution diagnostics 会区分本地 materialized record candidate/output/range-reject/filter-reject row 总数与 sampled execution rows，record execution sample 采样额度独立于 record output sample，execution sample 会记录 chunk-local row、file-local `local_input` ordinal、query range context、decoded value-column count 和 output/range-reject/filter-reject result，rejected row 标记为 `local_output=none`，record output sample 与 output execution sample 会记录 local output ordinal，并在 count-only decode path summary 输出 sampled record execution action counts，JSON decode path summary 也会输出 `record_execution_action_counts`。

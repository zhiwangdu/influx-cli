...
| mergeset | 已有 part metadata、metaindex row、index block-header、item payload summary、openGemini TSI/tag namespace item summary、openGemini field-index namespace item summary、openGemini CLV text-index item summary、payload-backed part/file-set table scan summary、ascending/descending part/file-set item search exact-match/exact-miss seek window simulation、ascending block-gap cursor advance accounting、TableSearch seek/heap cursor simulation、heap output part provenance、heap insert/pop/cursor-advance execution accounting、final deduped scan/search output samples 和 duplicate item merge-window/dedup summary；后续补充更完整本地 multi-part table cursor execution |

...
| 类型 | 说明 |
| --- | --- |
| tssp | openGemini 文件元数据、chunk metadata expansion、query-hit data block header probe、row-count materialization 和 one-row value samples |
| tssp-metaindex | openGemini detached `segment.idx` meta-index sidecar、chunk metadata batch planning、sibling `segment.meta` expansion、segment-level data ReadAt planning、sibling `segment.bin` range validation、query-hit data block CRC probe、row-count materialization 和 one-row value samples |

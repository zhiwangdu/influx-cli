...
| `internal/analysis/storage` | Phase 5 初始 storage file analyzer：TSM/WAL/TSSP/TSI 文件元数据、key/series 样例、block/meta-index/entry 统计、TSI index/log series-id set cardinality、TSM tombstone range/impact、TSM/WAL decode path estimate、TSSP chunk metadata、detached TSSP meta-index sidecar、query range overlap |

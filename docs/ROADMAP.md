...
| `internal/analysis/storage` | Phase 5 初始 storage file analyzer：TSM/TSSP 文件元数据、key/series 样例、block/meta-index 统计、query range overlap |
...
当前覆盖 InfluxDB TSM attached file metadata 和 openGemini attached TSSP trailer/meta-index metadata。TSI 深度解析、TSSP chunk metadata 完整解码、cursor simulation 和优化前后 decode path 对比仍属于后续 Phase 5 工作。

...
| `internal/analysis/storage` | Phase 5 初始 storage file analyzer：TSM/WAL/TSSP/TSI/series-file/fields-index/mergeset/opengemini-meta/opengemini-pk-meta/opengemini-pk-index/opengemini-bloom-filter/opengemini-text-index 文件元数据、key/series 样例、block/meta-index/entry 统计、TSI index/log series-id set cardinality、InfluxDB `_series` live/tombstone series summary、`fields.idx`/`fields.idxl` field type summary、TSM tombstone range/impact、TSM/WAL decode path estimate、TSM final optimized cursor output samples、WAL local write/delete replay candidate samples、TSSP chunk metadata、attached/detached TSSP column projection/simple/finite-set/string-ordered field predicate filtering/data block probe/filter row accounting、row-count materialization、one-row value samples、含 null bitmap 的普通 block samples、raw/old-gorilla/snappy/gorilla/same/RLE/MLF float full-block samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples、uncompressed/snappy/zstd/LZ4 string full-block samples、跨列 record samples、detached TSSP meta-index sidecar/chunk metadata batch planning/`segment.meta` expansion/data ReadAt planning/`segment.bin` range validation、openGemini detached `primary.meta` 和 attached `.idx` 主键 schema/meta-block/CRC/data range summary、openGemini bloom filter secondary index block/group/CRC summary、openGemini text secondary index `.pos`/`.bh`/`.ph` sidecar summary 和 declared/valid keys/post payload byte summary、mergeset metaindex/index block-header/item payload、openGemini TSI/tag namespace item summary、openGemini field-index namespace item summary、openGemini CLV text-index item summary、part/file-set table scan summary、file-set item search、TableSearch seek/heap cursor simulation、heap output part provenance、heap insert/pop execution accounting、final deduped scan/search output samples 和 duplicate item merge-window/dedup summary、openGemini meta topology snapshot summary、query range overlap |
...
当前覆盖 InfluxDB TSM attached file metadata、WAL write/delete/delete-range entry metadata、WAL local write/delete replay candidate samples、tombstone range/impact summary、基于 query range/key 的 TSM/WAL decode path estimate、per-file 和 FileStore-level ascending/descending cursor window/merge window simulation、优化前后 block/entry/byte/value decode path estimate、decoded timestamp output/dedup estimate、TSM value output comparison、local TSM KeyCursor-style execution stats/comparison output samples/final optimized cursor output samples、TSM FileStore.Cost-style file/block/byte estimate、TSI index file measurement/tag summary、TSI log entry summary、live/tombstone series-id set cardinality、InfluxDB `_series` series-file partition/segment entry/tombstone/key summary、`fields.idx`/`fields.idxl` measurement field-type summary 和 measurement/tag predicate inspection，以及 openGemini attached TSSP trailer/meta-index metadata、none/snappy/LZ4/self-compressed chunk metadata、per-file 和 file-set series-id filtered segment-level decode path estimate、TSSP ContainsValue/MetaIndex-style candidate cost estimate、ascending/descending TSSP LocationCursor-style segment window samples、TSSP ReadAt call estimate、`--column` projected optimized column-segment read ranges、file-set decoded output provenance/final exact dedup samples、`--field` 简单、有限集合和字符串有序字段谓词过滤及 decoded-row input/match/reject accounting、attached query-hit projected data block header probe、row-count materialization、one-row value samples、含 null bitmap 的普通 block samples、raw/old-gorilla/snappy/gorilla/same/RLE/MLF float full-block samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples、uncompressed/snappy/zstd/LZ4 string full-block samples、跨列 record samples、detached `segment.idx` meta-index CRC validation、query-range candidate filtering、detached chunk metadata batch planning、可选 sibling `segment.meta` expansion、detached segment-level data ReadAt planning、`--column` projected detached data probe、`--field` detached decoded-row simple/finite-set/string-ordered predicate filtering及 decoded-row input/match/reject accounting、可选 sibling `segment.bin` header/range validation、query-hit projected data block CRC/header probe、row-count materialization、one-row value samples、含 null bitmap 的普通 block samples、raw/old-gorilla/snappy/gorilla/same/RLE/MLF float full-block samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples、uncompressed/snappy/zstd/LZ4 string full-block samples、跨列 record samples、openGemini detached `primary.meta` 和 attached `.idx` 主键 schema/meta-block/CRC/data range summary、openGemini bloom filter secondary index block/group/CRC summary、openGemini text secondary index `.pos`/`.bh`/`.ph` sidecar summary 和 declared/valid keys/post payload byte summary、mergeset part metadata/metaindex/index block-header/item payload summary、openGemini TSI/tag namespace item summary、openGemini field-index namespace item summary、openGemini CLV text-index item summary、payload-backed part/file-set table scan summary、file-set item search simulation、TableSearch seek/heap cursor simulation、heap output part provenance、heap insert/pop execution accounting、final deduped scan/search output samples、duplicate item merge-window/dedup summary、openGemini meta topology snapshot protobuf/JSON summary。完整本地 storage cursor simulation/execution 和更完整的 filter 表达式执行仍属于后续 Phase 5 工作。
...
| tssp | openGemini 文件元数据、chunk metadata expansion、`--column` projected ReadAt planning、file-set decoded output provenance/final exact dedup samples、`--field` simple/finite-set/string-ordered decoded-row predicate filtering/input-match-reject accounting、query-hit projected data block header probe、row-count materialization、one-row value samples、含 null bitmap 的普通 block samples、raw/old-gorilla/snappy/gorilla/same/RLE/MLF float full-block samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples、uncompressed/snappy/zstd/LZ4 string full-block samples 和跨列 record samples |
| tssp-metaindex | openGemini detached `segment.idx` meta-index sidecar、chunk metadata batch planning、sibling `segment.meta` expansion、`--column` projected segment-level data ReadAt planning、`--field` simple/finite-set/string-ordered decoded-row predicate filtering/input-match-reject accounting、sibling `segment.bin` range validation、query-hit projected data block CRC/header probe、row-count materialization、one-row value samples、含 null bitmap 的普通 block samples、raw/old-gorilla/snappy/gorilla/same/RLE/MLF float full-block samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples、uncompressed/snappy/zstd/LZ4 string full-block samples 和跨列 record samples |
...
| opengemini-pk-index | 已有本地 openGemini attached colstore 主键 `.idx` 解析，输出 `COLX` header、attached meta size、row count、主键 schema、time-cluster location、inline column data offset/range summary；不解码 record，不调用 `PrimaryKeyReader`、fileops runtime、engine 或数据库服务 |
...
| opengemini-text-index | 已有本地 openGemini text secondary index sidecar 解析，attached `.ph` part header 输出 part/block-header 声明，sibling `.bh` 解码可变长 block header，sibling `.pos` 做 keys/post payload offset/size 边界校验和 declared/valid payload byte 汇总；不解压 posting list，不调用 `TextIndexFilterReader`、fileops/OBS runtime、engine 或数据库服务 |
| mergeset | 已有 part metadata、metaindex row、index block-header、item payload summary、openGemini TSI/tag namespace item summary、openGemini field-index namespace item summary、openGemini CLV text-index item summary、payload-backed part/file-set table scan summary、ascending/descending file-set item search simulation、TableSearch seek/heap cursor simulation、heap output part provenance、heap insert/pop execution accounting、final deduped scan/search output samples 和 duplicate item merge-window/dedup summary；后续补充更完整本地 multi-part table cursor execution |

...
| 类型 | 说明 |
| --- | --- |
| tssp | openGemini 文件元数据、chunk metadata expansion、query-hit data block header probe、row-count materialization、one-row value samples、uncompressed/const-delta/simple8b integer full-block samples、bitpack boolean full-block samples 和 uncompressed string full-block samples |

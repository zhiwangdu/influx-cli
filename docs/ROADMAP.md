...
| mergeset | 已有 part metadata、metaindex row、index block-header、item payload summary、payload-backed table scan summary、file-set item search simulation、TableSearch seek/heap simulation 和 duplicate item merge/dedup summary；后续补充完整 storage-engine-backed multi-part table cursor execution |

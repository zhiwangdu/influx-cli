...
| opengemini-text-index | 已有本地 openGemini text secondary index sidecar 解析，attached `.ph` part header 输出 part/block-header 声明，sibling `.bh` 解码可变长 block header，sibling `.pos` 做 keys/post payload offset/size 边界校验和声明 payload byte 汇总；不解压 posting list，不调用 `TextIndexFilterReader`、fileops/OBS runtime、engine 或数据库服务 |

...
| tssp-metaindex | openGemini detached `segment.idx` meta-index sidecar、chunk metadata batch planning、sibling `segment.meta` expansion、segment-level data ReadAt planning、sibling `segment.bin` range validation 和 query-hit data block CRC probe |

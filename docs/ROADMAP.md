...
TSSP multi-word field predicate operator alias 会接受 hyphen、space 和 underscore 分隔符，bang-negated prefix/suffix alias 也接受 underscore 形式；record execution diagnostics 会区分本地 materialized record output row 总数与 sampled execution rows，sample 会记录 query range context、decoded value-column count 和 local output ordinal。

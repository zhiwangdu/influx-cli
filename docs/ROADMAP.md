...
| 类型 | 说明 |
| --- | --- |
| tssp | openGemini 文件元数据、chunk metadata expansion、query-hit data block header probe、row-count materialization、one-row value samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples 和 uncompressed string full-block samples |
| tssp-metaindex | openGemini detached `segment.idx` meta-index sidecar、chunk metadata batch planning、sibling `segment.meta` expansion、segment-level data ReadAt planning、sibling `segment.bin` range validation、query-hit data block CRC probe、row-count materialization、one-row value samples、uncompressed/const-delta/simple8b/zstd integer full-block samples、bitpack boolean full-block samples 和 uncompressed string full-block samples |

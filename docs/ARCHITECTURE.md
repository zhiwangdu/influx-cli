...
openGemini mergeset analyzer 先覆盖 part directory metadata：解析 `items_blocks_suffix` part 名、`metadata.json`、以及 `metaindex.bin`/`index.bin`/`items.bin`/`lens.bin` component size。metaindex row 和 index block 深度解码仍属于后续 Phase 5。

...
TSSP JSON decode path summaries expose `record_execution_action_counts` as a sampled local record execution action-count map, so count-oriented table/report output can summarize output, range-reject, and filter-reject record steps without depending on raw sampled row values.

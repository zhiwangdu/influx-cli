...
openGemini mergeset analyzer 先覆盖 part directory metadata、metaindex row summary、index block-header summary 和 item payload summary：解析 `items_blocks_suffix` part 名、`metadata.json`、`metaindex.bin` zstd row、`index.bin` zstd block header、以及 `items.bin`/`lens.bin` plain/zstd item payload，校验 header range、decoded item count、first/last item 和排序。完整 item cursor/search simulation 仍属于后续 Phase 5。

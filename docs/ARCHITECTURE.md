...
TSSP record execution sample 会记录每个本地 materialized record row 的 query range context 和 decoded value-column count，便于区分单列 value sample、跨列 record materialization 与查询窗口。

...
TSSP `--field` 只在本地已解码 data block value 上执行有限谓词：`=`/`!=` 按 decoded value 类型比较，`in`/`not-in` 对逗号分隔集合执行同样的 typed membership 比较，`null` 作为 decoded null row 的哨兵用于 `=null`、`!=null` 和集合匹配，`>`/`>=`/`<`/`<=` 对 decoded integer/float block 做数值比较、对 decoded string block 做字典序比较，并输出本地 decoded row 的 input/match/reject 计数；它不调用 openGemini filter executor、record runtime、engine、HTTP API 或数据库服务。

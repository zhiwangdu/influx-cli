...
TSSP record execution sample 会记录每个本地 materialized record row 的 query range context、decoded value-column count 和 local output ordinal，便于区分单列 value sample、跨列 record materialization、查询窗口与本地输出顺序。

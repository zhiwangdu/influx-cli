...
openGemini TSSP analyzer 先覆盖 attached 文件 trailer/meta-index/chunk metadata，以及 detached `segment.idx` meta-index sidecar。attached 文件在 chunk metadata expanded 且 trailer 声明 data section 时，会按 query 命中的 column segment 做 data block header probe、可证明 header 类型的 row-count materialization、`Block*One` value sample extraction、`BlockIntegerFull` uncompressed/const-delta/simple8b/zstd integer sample extraction、`BlockBooleanFull` bitpack boolean sample extraction 和 `BlockStringFull` uncompressed string sample extraction。detached sidecar 做本地结构解析、CRC 校验、query-range candidate filtering、`ChunkMetaReadNum` 风格的 chunk metadata batch planning，并在同目录存在 `segment.meta` 时按 openGemini detached chunk-meta record CRC 解码 chunk metadata，进一步用 chunk metadata 规划 detached segment-level data ReadAt 范围；若同目录存在 `segment.bin`，还会校验 data file header、column segment offset/size 是否落在文件内，并对 query 命中的 detached data block 做 ReadAt CRC/header probe、可证明 header 类型的 row-count materialization、`Block*One` value sample extraction、`BlockIntegerFull` uncompressed/const-delta/simple8b/zstd integer sample extraction、`BlockBooleanFull` bitpack boolean sample extraction 和 `BlockStringFull` uncompressed string sample extraction。完整 codec-backed TSSP value record materialization 仍属于后续 Phase 5。

...
openGemini TSSP analyzer 先覆盖 attached 文件 trailer/meta-index/chunk metadata，以及 detached `segment.idx` meta-index sidecar。detached sidecar 只做本地结构解析、CRC 校验、query-range candidate filtering 和 `ChunkMetaReadNum` 风格的 chunk metadata batch planning；完整 detached data/chunk record reader 执行路径仍属于后续 Phase 5。

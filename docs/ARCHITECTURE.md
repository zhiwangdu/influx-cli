...
TSSP 多词 field predicate operator alias 会把 hyphen、space 和 underscore 分隔符归一到同一个本地 operator，例如 `not-like`、`not like` 和 `not_like` 等价；`not-exists`、`not exists`、`not_exists` 和 `!exists` 会归一到同一个本地 existence operator；`matches`/`match`/`regex`/`regexp` 及其 `not`/`!` 否定别名会归一到本地 decoded-row regex operator `=~`/`!~`；大小写不敏感 string alias `ilike`、`icontains`、`istarts-with` 和 `iends-with` 也走同一条本地 decoded-row string-only predicate 路径；bang-negated prefix/suffix alias 也接受 underscore 形式，例如 `!starts_with`、`!istarts_with`、`!ends_with` 和 `!iends_with`。归一化只影响本地 decoded-row predicate 解释，不调用外部 parser 或查询执行器。

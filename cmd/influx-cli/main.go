...
func newStorageCommand(flags *globalFlags) *cobra.Command {
	...
	analyzeCommand.Flags().StringVar(&analyzeFlags.format, "storage-format", analyzeFlags.format, "storage file format: auto, tsm, wal, tssp, tssp-metaindex, tsi, tsi-log, series-file, fields-index, mergeset, opengemini-meta")

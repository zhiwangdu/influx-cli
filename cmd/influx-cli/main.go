...
func newStorageCommand(flags *globalFlags) *cobra.Command {
	...
	analyzeCommand.Flags().StringArrayVar(&analyzeFlags.fields, "field", nil, "TSSP decoded field predicate as key=value, key==value, key!=value, key<>value, key=~regex, key!~regex, key is value, key is-not value, key is not value, key>value, key>=value, key<value, key<=value, key in (value1,value2), key not-in (value1,value2), key between (lower,upper), key not-between (lower,upper), key contains value, key not-contains value, key starts-with value, key not-starts-with value, key ends-with value, or key not-ends-with value for local record filtering, including decoded time when present; range parentheses are optional; quote string values that contain commas or parentheses; repeat for multiple fields; requires --from/--to")
